...
export const proxmoxTemplates = pgTable('proxmox_templates', {
  ...
  techStacks: text('tech_stacks').$type<string[]>().default(JSON.stringify([])), // New tech stacks added to this template
  inheritedTechStacks: text('inherited_tech_stacks').$type<string[]>().default(JSON.stringify([])), // Tech stacks inherited from parent
  isDefault: boolean('is_default').default(false).notNull(),
  ...
  envVars: text('env_vars').$type<EnvVarsJson>().default(JSON.stringify({})), // Environment variables for containers
  createdAt: integer('created_at').$defaultFn(() => Date.now()).notNull(),
  updatedAt: integer('updated_at').$defaultFn(() => Date.now()).notNull(),
});
...
export const repositories = pgTable('repositories', {
  ...
  techStack: text('tech_stack').$type<string[]>().default(JSON.stringify([])), // Tech stack IDs to install on workspaces (override template)
  envVars: text('env_vars').$type<EnvVarsJson>().default(JSON.stringify({})), // Environment variables for containers (overrides template)
  gitHooks: text('git_hooks').$type<GitHooksJson>().default(JSON.stringify({})), // Git hooks to sync to workspaces
  // Cached branch info from remote (fetched via git ls-remote)
  cachedBranches: text('cached_branches').$type<string[]>().default(JSON.stringify([])),
...
export const tabs = pgTable('tabs', {
  ...
  command: text('command').$type<string[]>().default(JSON.stringify(['/bin/bash'])), // Command to exec
  exitOnClose: boolean('exit_on_close').default(false).notNull(), // Append && exit to command
  outputBuffer: text('output_buffer').$type<string[]>().default(JSON.stringify([])),
...
export const tabTemplates = pgTable('tab_templates', {
  ...
  args: text('args').$type<string[]>().default(JSON.stringify([])), // Command arguments
...
export const secrets = pgTable('secrets', {
  ...
  description: text('description'),
  templateWhitelist: text('template_whitelist').$type<string[]>().default(JSON.stringify([])).notNull(),
  createdAt: integer('created_at').$defaultFn(() => Date.now()).notNull(),
  updatedAt: integer('updated_at').$defaultFn(() => Date.now()).notNull(),
}, (table) => ({
...
export const workspaceShares = pgTable('workspace_shares', {
  ...
  sharedByUserId: uuid('shared_by_user_id')
    .references(() => users.id, { onDelete: 'cascade' })
    .notNull(),
  permissions: text('permissions').$type<string[]>().default(JSON.stringify(['view', 'execute'])).notNull(),
  createdAt: integer('created_at').$defaultFn(() => Date.now()).notNull(),
  updatedAt: integer('updated_at').$defaultFn(() => Date.now()).notNull(),
}, (table) => ({
