...
export async function applyEnvVarChanges(
  ...
  // Step 1: Update /etc/profile.d/session-hub-env.sh
...
/**
 * Write all env vars to /etc/profile.d/session-hub-env.sh
...
`;
  ...
  try {
    ...
    await execAsync(`sudo bash -c 'cat > /etc/profile.d/session-hub-env.sh << '\\''ENVEOF'\\''
${envContent}
ENVEOF
chmod 644 /etc/profile.d/session-hub-env.sh'`);

    console.log(`Updated /etc/profile.d/session-hub-env.sh with ${Object.keys(envVars).length} variables`);
  } catch (error) {
    console.error('Failed to update /etc/profile.d/session-hub-env.sh:', error);
